...
public class OPFinalizacaoVenda extends Operacao implements IOperacao {
	...
	private IECFService eCFService = ECFServiceFactory.getInstance()
			.criarECFService();
	...
	protected Map efetuar(Map parametros) throws OperacaoException {
		...
		lsParcelaPagamento = (List<VOParcelaPagamento>) parametros
				.get("lsParcelaPagamento");
		venda = (BeanVenda) parametros.get("BeanVenda");

		Operacao executarTef = OperacaoFactory.getInstance()
				.criarOPExecutarTef();
		parametros = executarTef.executar(parametros);

		lsTransacoesTef = (List<VOTransacaoTef>) parametros
				.get("lsTransacoesTef");
		...
		if ((Boolean) parametros.get("BooleanResult")) {
			...
			if (pagamentoOK) {
				...
				BeanOperador beanOperador = (BeanOperador) parametros
						.get("BeanOperador");
				BeanCliente beanCliente = (BeanCliente) parametros
						.get("BeanCliente");
				BeanCondicaoPagamento beanCondicaoPagamento = (BeanCondicaoPagamento) parametros
						.get("BeanCondicaoPagamento");
				BigDecimal troco = (BigDecimal) parametros.get("valorTroco");

				for (VOParcelaPagamento pp : lsParcelaPagamento) {
					for (VOItemParcelaPagamento ipp : pp
							.getLsItemParcelaPagamento()) {
						...
						if (ipp.getFormaPagamento().getTipo().equals("DEV")) {
							...
							DAOFacade.atualizarDevolucaoUtilizada(devolucao,
									true);
						}
						...
						lancamento.setCodFormaPagamento(ipp.getFormaPagamento()
								.getCodFormaPagto());
						lancamento.setTipoFormaPagamento(ipp
								.getFormaPagamento().getTipo());
						lancamento.setCodConveniada(ipp.getConveniada()
								.getCodConveniada());
						lancamento
								.setCodCliente(beanCliente != null ? beanCliente
										.getCodCliente() : null);

						/* Caso seja dinheiro, subtrair o valor do troco */// ---
																			// MELHORAR
																			// -
																			// PEGAR
																			// ULTIMO
						...
						if (lancamento.getTipoFormaPagamento().equals("DIN")
								&& troco.compareTo(BigDecimal.ZERO) > 0) {
						...
						lancamento
								.setTaxaAdm(ipp.getConveniada().getTaxaAdm() != null ? ipp
										.getConveniada().getTaxaAdm()
										: BigDecimal.ZERO);
						lancamento.setNumeroAutorizacao(ipp
								.getNumeroAutorizacao());

						BigDecimal valorLiquido = lancamento.getValorBruto();
						if (lancamento.getTaxaAdm().compareTo(BigDecimal.ZERO) > 0) {
							BigDecimal rMult = lancamento.getValorBruto()
									.multiply(lancamento.getTaxaAdm())
									.setScale(2, BigDecimal.ROUND_HALF_UP);
							BigDecimal rDiv = rMult
									.divide(new BigDecimal(100d));
							valorLiquido = lancamento.getValorBruto()
									.subtract(rDiv)
									.setScale(2, BigDecimal.ROUND_HALF_UP);
						}
						...
						lancamento
								.setCodCondicaoPagamento(beanCondicaoPagamento
										.getCodCondPagto());
						lancamento.setNumLancamento(ipp.getIndiceItemParcela());
						lancamento.setNumParcela(String.valueOf(pp
								.getNumeroParcela()));
						...
						if (!ipp.getCheque().isEmpty()) {
							lancamento.setChqBanco(ipp.getCheque()
									.getCodBanco());
							lancamento.setChqAgencia(ipp.getCheque()
									.getCodAgencia());
							lancamento.setChqConta(ipp.getCheque()
									.getContaCorrente());
							lancamento.setChqNumero(ipp.getCheque()
									.getNumeroCheque());
						}
				...
				beanVendaNovo.setValorSubTotal((BigDecimal) parametros
						.get("SubTotalVenda"));
				beanVendaNovo.setValorTotal((BigDecimal) parametros
						.get("TotalVenda"));
				...
				/*
				 * Caso o cupom já tenha sido fechado e a ECF esteja desligada,
				 * sair da aplicação
				 */
				if (cupomFechado && !eCFService.getECFAtiva()) {
					JOptionPane.showMessageDialog(null,
							"O sistema será encerrado.", "Aviso do Sistema",
	...
	private void cancelarUltimoCupom() {
		Operacao cancelaVenda = OperacaoFactory.getInstance()
				.criarOPCancelaVenda();
	...
	private void confirmarRecebimentos() {
		...
		if (eCFService.getECFAtiva()) {
			...
			int escolha = JOptionPane.showOptionDialog(null,
					"Erro na impressão. Deseja tentar novamente?",
	...
	private void subtotalizarCupom() {
		try {
			...
			if (valorDesconto.compareTo(BigDecimal.ZERO) > 0) {
				desconto = Integer.parseInt(String.valueOf(-1
	...
	private void efetuarFormasPagamento() {
		try {
			...
			Iterator itPagamento = tiposPagamentosUtilizados.keySet()
					.iterator();
			while (itPagamento.hasNext()) {
				String keyPagamento = (String) itPagamento.next();
				VOEfetuaPagamentoECF formaPagamentoECF = (VOEfetuaPagamentoECF) tiposPagamentosUtilizados
						.get(keyPagamento);

				String dFormaPagamento = keyPagamento.substring(0,
						keyPagamento.indexOf("="));

				// if (Caixa.cupomAberto) {
				if (!dFormaPagamento.equalsIgnoreCase("DINHEIRO")) {
					eCFService.efetuarPagamento(dFormaPagamento,
							formaPagamentoECF.getTotalTipo(), formaPagamentoECF
			...
			if (!indiceDinheiro.equals("")
					&& totalDinheiro.compareTo(BigDecimal.ZERO) > 0) {
	...
	private void fecharCupomFiscal() {
		try {
			...
			// }
			/*
			 * Quando fechar o cupom deve registrar num booleano para em caso de
			 * erro, conseguir cancelar o cupom
			 */
			cupomFechado = true;
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void cancelarRecebimentos(boolean auto) {
		...
		if (!auto) {
			escolha = JOptionPane
					.showOptionDialog(
							null,
							"Confirmar o cancelamento dos pagamentos?"
									+ (char) 13
									+ (char) 10
									+ "Se houver pagamento com TEF, o pagamento com TEF será cancelado!,"
									+ (char) 13 + (char) 10
									+ "Deseja Continuar?", "Encerrar venda.",
	...
	private Map agruparValorPorTipo() {
		...
		for (VOParcelaPagamento pp : lsParcelaPagamento) {
			for (VOItemParcelaPagamento ipp : pp.getLsItemParcelaPagamento()) {
				...
				efetuaPagamentoVO = (VOEfetuaPagamentoECF) pagamentosUtilizados
						.get(keyString);
				...
				if (efetuaPagamentoVO != null) {
					...
					if (!ipp.getConveniada().isEmpty()) {
						efetuaPagamentoVO.setDescricaoConveniada(ipp
								.getConveniada().getDescricao());
					}

					efetuaPagamentoVO.setTotalTipo(efetuaPagamentoVO
							.getTotalTipo().add(totalTipo));
					efetuaPagamentoVO.setQuantidadeParcelas(efetuaPagamentoVO
							.getQuantidadeParcelas() + 1);
				} else {
					efetuaPagamentoVO = new VOEfetuaPagamentoECF();
					efetuaPagamentoVO.setDescricaoFormaPagamento(ipp
							.getFormaPagamento().getDescricao());
	...
	private void tratarErrosConfirmacaoRecebimentos(Throwable t)
	...
	private int tratarErroImpressoraNaoResponde(Throwable ex)
			throws HeadlessException {
		// User32 user32 = NLink.create(User32.class);
		// user32.BlockInput(false);
		ex.printStackTrace();
		String[] opcoes = { "Sim", "Não" };
		int escolha = JOptionPane.showOptionDialog(null,
				"Impressora não responde. Deseja continuar?",
				"Aviso do Sistema", JOptionPane.YES_NO_OPTION,
				JOptionPane.QUESTION_MESSAGE, null, opcoes, null);
		return escolha;
	}
	...
	private boolean imprimirTransacoesTEF(boolean estado) {
		...
		Operacao relComrovante1 = OperacaoFactory.getInstance()
				.criarOPRelatorioComprovanteTEF1();
		parametro = relComrovante1.executar(parametro);

		// Operacao relComrovante2 =
		// OperacaoFactory.getInstance().criarOPRelatorioComprovanteTEF2();
