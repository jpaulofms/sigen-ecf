...
public class SigenApplication {
    ...
    private static void movimentoAbertura() {
        try {
            ...
            if (!eCFService.getECFAtiva()) {
                JOptionPane.showMessageDialog(null, "ECF desligada/não encontrada. Sistema será fechado", "Estado ECF", JOptionPane.WARNING_MESSAGE);
                System.exit(0);
            }
            ...
            List<BeanMovimento> lsMovimentoDia = DAOFacade.getLsMovimento(beanMovimento, clausula);

            if (lsMovimentoDia.isEmpty() || (!lsMovimentoDia.isEmpty() && lsMovimentoDia.get(0).getDataFechamento() != null)) {
                BeanMovimento movimentoAnterior = DAOFacade.getMovimentoAnterior(new BeanMovimento());
            ...
            if (!recuperarVenda()) {
                ...
                if (eCFService.getECFCupomAberto()) {
                    JOptionPane.showMessageDialog(null, "Existe um cupom aberto que será cancelado", "Estado ECF", JOptionPane.WARNING_MESSAGE);
                    eCFService.cancelaCupom();
                }
            }

            /* Ações */
            Operacao abrirAplicacao = OperacaoFactory.getInstance().criarOPAbrirAplicacao();
            parametros = abrirAplicacao.executar(parametros);

            /* inicia caixa */
            VIEWCaixa caixaVIEW = new VIEWCaixa();
            caixaVIEW.setLabelOperador((BeanOperador) parametros.get("BeanOperador"));
    ...
    private static Map iniciarMovimento(Map parametros) {
        /* Abrir Movimento */
        VIEWLoginIniciarMovimento loginIniciarMovimento = new VIEWLoginIniciarMovimento(null, true, parametros);
    ...
    private static Map movimentoAberto(Map parametros) {
        /* Movimento Aberto */
        VIEWLoginIniciarMovimentoAberto iniciarMovimentoAberto = new VIEWLoginIniciarMovimentoAberto(null, true, parametros, false);
    ...
    private static boolean recuperarVenda() {
        ...
        try {
            ...
            vendaNaoFinalizada.setCoo(ECFServiceFactory.getInstance().criarECFService().getCOO());

            String clausula = " ORDER BY CAST(COD_MOVIMENTO AS INTEGER) DESC";
            vendaNaoFinalizada = DAOFacade.getUltimaVenda(vendaNaoFinalizada, clausula);

            if (vendaNaoFinalizada == null || vendaNaoFinalizada.getValorTotal().compareTo(BigDecimal.ZERO) > 0) {
                return false;
            }

            String[] opcoes = {"Sim", "Não"};
            int escolha = JOptionPane.showOptionDialog(null, "Existe uma venda que não foi finalizada, deseja tentar recuperá-la ?", "Recuperação de Venda",
            ...
            parametros.put("CupomRecuperarAberto", eCFService.getECFCupomAberto());
            parametros.put("BeanVendaRecuperar", vendaNaoFinalizada);

            return true;
        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(null, "Erro na recuperação da venda. Cupom será cancelado", "Recuperação de Venda", JOptionPane.WARNING_MESSAGE);
            return false;
        }
