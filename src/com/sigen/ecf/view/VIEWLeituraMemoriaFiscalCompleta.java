...
public class VIEWLeituraMemoriaFiscalCompleta extends javax.swing.JDialog {

    public VIEWLeituraMemoriaFiscalCompleta(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        setTitle("LMFC - Leitura da Memória Fiscal Completa");
        setModal(true);
        setResizable(false);
    ...
    private boolean confirma(String nInicial, String nFinal) {
        ...
        try {
            ...
            if (opcao1.isSelected()) {
                if (radioPeriodo.isSelected()) {
                    // DATA
                    eCFService.LMFDataEmitir(nInicial, nFinal, "c");
                } else if (radioCRZ.isSelected()) {
                    // REDUÇÃO
                    eCFService.LMFReducaoEmitir(nInicial, nFinal, "c");
                }
            }
            ...
            if (opcao2.isSelected()) {
                // DATA
                if (radioPeriodo.isSelected()) {
                    eCFService.LMFDataGerar(nInicial, nFinal, "c");
                } else if (radioCRZ.isSelected()) {
                    // REDUÇÃO
                    eCFService.LMFReducaoGerar(nInicial, nFinal, "c");
                }
            }
            return true;
        } catch (Exception ex) {
