...
public class VIEWEspelhoMFD extends javax.swing.JDialog {

    public VIEWEspelhoMFD(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void gerarEspelhoMFD() {
        if (validarDados()) {
            JOptionPane.showMessageDialog(this, "Espelho MFD gerado com sucesso!", "Espelho MFD", JOptionPane.OK_OPTION);
            dispose();
        }
    ...
    private boolean confirma(String nInicial, String nFinal) {
        ...
        try {
            ...
            eCFService.gerarEspelho(nInicial, nFinal, tipo, "1");

            return true;
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Erro na geração de Espelho MFD!", "Espelho MFD", JOptionPane.WARNING_MESSAGE);
            return false;
        }
    }
}
